...
import { tenants, users, outlets, modules, tenantModules } from '../models/schema.js';
...
router.post('/modules/toggle', requireAuth, requireTenantBound, requireRole(['owner']), async (req, res) => {
    ...
    try {
        ...
        const existingTenantModule = await db.select()
            .from(tenantModules)
            .where(and(eq(tenantModules.tenantId, tenantId), eq(tenantModules.moduleId, data.moduleId)));
